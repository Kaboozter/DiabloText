...
void ConsumableItem::UseItem(Player& aPlayer, Log& aLog) 
{
	if (Materials::myAmount > 0) {
		aPlayer.myHp += (aPlayer.myMaxHp * myPotency);
		Materials::myAmount--;
		aLog.Write(std::to_string(Materials::myAmount) + " Potions Left");
	}
	else {
...
ConsumableItem::ConsumableItem(int aQual) {
	myAmount = 0;
	myPotency = ((aQual + 1) * 25);
}

Materials::Materials(int aType) {
	myAmount = 0;
	if (myType == 0) {
		myName = "Goblin Tooth";
